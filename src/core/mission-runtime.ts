...
export class MissionRuntime {
      ...
      for (const step of mission.plan) {
        ...
        if (this.provider && output !== undefined) {
          ...
          if (decision.action === "call_tool" && decision.tool) {
            const followUp = this.tools.get(decision.tool);
            await this.runTool(mission, step, followUp.name, decision.input, workspace);
          }
        }
      }
  ...
  private async runStep(mission: Mission, step: MissionStep, workspace: string): Promise<unknown> {
    ...
    let output: unknown;
    if (step.tool) output = await this.runTool(mission, step, step.tool, { goal: mission.goal, step: step.description }, workspace);
