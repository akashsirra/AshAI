...
function parseInput(raw: unknown): unknown { if (typeof raw !== "string" || !raw.trim()) return undefined; try { return JSON.parse(raw); } catch { throw new Error("Groq returned invalid tool input JSON."); } }
...
export class GroqProvider implements ModelProvider {
  ...
  async plan(goal: string): Promise<MissionStep[]> {
    ...
    return parsed.steps.map((item, index) => { if (!item || typeof item !== "object") throw new Error(`Invalid plan step ${index}.`); const value = item as Record<string, unknown>; const tool = typeof value.tool === "string" && TOOL_NAMES.includes(value.tool as ToolName) ? value.tool as ToolName : undefined; return { id: typeof value.id === "string" ? value.id : `step-${index + 1}`, title: typeof value.title === "string" ? value.title : `Step ${index + 1}`, description: typeof value.description === "string" ? value.description : goal, status: "pending" as const, tool, input: parseInput(value.input), requiresApproval: tool === "workspace.write_file" }; });
  }
  ...
  async decide(input: { goal: string; step: MissionStep; toolResult?: unknown }): Promise<Awaited<ReturnType<ModelProvider["decide"]>>> {
    ...
    return { action: action as AgentAction, tool, input: parseInput(decision.input), summary: typeof decision.summary === "string" ? decision.summary : undefined };
  }
