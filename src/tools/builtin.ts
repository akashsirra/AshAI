...
async function collectFiles(workspace: string, requested: string, max: number): Promise<{ files: string[]; resolvedPath: string; warning?: string }> {
  const root = resolve(workspace);
  const requestedPath = safePath(workspace, requested);
  const start = await pathExists(requestedPath) ? requestedPath : root;
  ...
  const prefix = start === root ? "" : relative(root, start);
...
export const workspaceInspect: ToolDefinition = {
  ...
  async execute(input, context) {
    ...
    const requested = stringField(value, "path", "directory", "dir") ?? ".";
...
export const workspaceFindFiles: ToolDefinition = {
  ...
  description: "Recursively list workspace files. If a requested directory is missing, automatically fall back to the workspace root and report the fallback.",
  async execute(input, context) {
    const value = objectInput(input);
    const requested = stringField(value, "path", "directory", "dir") ?? ".";
    const maxRaw = Number(value.max ?? value.limit ?? 300);
    const max = Math.min(1000, Math.max(1, Number.isFinite(maxRaw) ? maxRaw : 300));
    const result = await collectFiles(context.workspace, requested, max);
    return { path: requested, resolvedPath: result.resolvedPath, count: result.files.length, truncated: result.files.length >= max, ...(result.warning ? { warning: result.warning } : {}), files: result.files };
  },
};

export const workspaceReadFile: ToolDefinition = {
  name: "workspace.read_file",
  description: "Read a UTF-8 text file inside the mission workspace. If the requested path is missing, locate a unique file with the same basename and read that instead.",
  async execute(input, context) {
    const value = objectInput(input);
    const requested = stringField(value, "path", "file", "filePath") ?? "README.md";
    const target = safePath(context.workspace, requested);
    if (await pathExists(target)) {
      const info = await stat(target);
      if (!info.isFile()) throw new Error(`Not a file: ${requested}`);
      return await readFile(target, "utf8");
    }

    const wanted = basename(requested);
    const candidates = (await collectFiles(context.workspace, ".", 1000)).files.filter(path => basename(path) === wanted);
    if (candidates.length === 1) {
      return `Resolved requested path '${requested}' to '${candidates[0]}'.\n\n${await readFile(safePath(context.workspace, candidates[0]), "utf8")}`;
    }
...
export const workspaceSearch: ToolDefinition = {
  ...
  async execute(input, context) {
    const value = objectInput(input);
    const pattern = stringField(value, "pattern", "query", "search");
    if (!pattern) throw new Error("workspace.search requires pattern");
    const regex = Boolean(value.regex);
    const maxRaw = Number(value.max ?? value.limit ?? 50);
    const max = Math.min(200, Math.max(1, Number.isFinite(maxRaw) ? maxRaw : 50));
...
export const workspaceExecute: ToolDefinition = {
  ...
  async execute(input, context) {
    const value = objectInput(input);
    const command = stringField(value, "command", "cmd") ?? "git status --short";
...
export const workspaceVerify: ToolDefinition = {
  ...
  async execute(input, context) {
    const value = objectInput(input);
    const command = stringField(value, "command", "cmd") ?? "npm run typecheck";
